...
static void init() {
    ...
    want.channels = 2;
    want.samples = 1024; // 小さいほど操作が追従しやすい(ただし負荷は増える)
...
static void update() {
    ...
    if (getKeyDown(SDL_SCANCODE_UP)) {
        ...
        // 左＝クリックを「早める」(位相を戻す)
        shift_phase_ms_locked(&st, -framesStep);
    }
...
static void draw() {
    ...
    DFA_DrawText(text, 10, 0, infoText.scale, 0, infoText.color, &layoutLeftCenter, info);
    DFA_Update(64);
